...
public class c_powerupSpawn : MonoBehaviour {
	...
	void SpawnPowerups() {
		for(int i = 0; i < i_powerups2Spawn; i++) {
			...
			print(f_y);
			GameObject go_test = (GameObject)Instantiate(go_powerup,new Vector3(c_waypoint.l_waypoints[i_prevWaypoint-5].x,c_terraingen.SampleTerrain(c_waypoint.l_waypoints[i_prevWaypoint-5],c_terraingen.f_trackRoughness)*c_terraingen.i_yRes+1f,c_waypoint.l_waypoints[i_prevWaypoint-5].y),Quaternion.identity);
			go_test.GetComponent<PowerUpController>().t_parent = transform;
		}
	}
}
